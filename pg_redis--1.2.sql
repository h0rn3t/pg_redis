...
--   * crash-safety release: shared-memory sizing widened to Size, eager DSA
--     attach, lock-free cold-start load, two-phase dirty-ring reservation,
--     commit-ordered drain, TOAST detoast on load, configurable bgworker DB.
--   * on-disk schema and TLV format UNCHANGED; in-shmem HTAB layout changed
--     (HASH_PARTITION dropped) so a clean cluster restart is required.
--   * adds crash-safety diagnostic functions (RING_INSPECT, LOAD_STATE,
--     LWLOCK_DIAG).
--
